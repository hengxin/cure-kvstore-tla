...
\begin{document}
\tlatex
\@x{}\moduleLeftDash\@xx{ {\MODULE} Cure}\moduleRightDash\@xx{}%
\begin{lcom}{0}%
...
\@xx{}%
\@pvspace{8.0pt}%
\@x{ cVars\@s{3.57} \.{\defeq} {\langle} cvc {\rangle}}%
 \@x{ sVars\@s{4.08} \.{\defeq} {\langle} clock ,\, pvc ,\, css ,\, store ,\,
 L {\rangle}}%
\@x{ mVars \.{\defeq} {\langle} msgs ,\, incoming {\rangle}}%
 \@x{ vars \.{\defeq} {\langle} cvc ,\, clock ,\, pvc ,\, css ,\, store ,\, L
 ,\, msgs ,\, incoming {\rangle}}%
\@x{}\midbar\@xx{}%
\@x{ VC \.{\defeq} [ Datacenter \.{\rightarrow} Nat ]\@s{4.1}}%
\@y{\@s{0}%
 vector clock with an entry per datacenter \ensuremath{d \.{\in} Datacenter
}}%
\@xx{}%
\@x{ VCInit \.{\defeq} [ d \.{\in} Datacenter \.{\mapsto} 0 ]}%
 \@x{ Merge ( vc1 ,\, vc2 ) \.{\defeq} [ d \.{\in} Datacenter \.{\mapsto} Max
 ( vc1 [ d ] ,\, vc2 [ d ] ) ]}%
\@pvspace{8.0pt}%
\@x{ DC \.{\defeq} Cardinality ( Datacenter )}%
 \@x{ DCIndex \.{\defeq} {\CHOOSE} f \.{\in} [ 1 \.{\dotdot} DC
 \.{\rightarrow} Datacenter ] \.{:} Injective ( f )}%
\@x{ LTE ( vc1 ,\, vc2 ) \.{\defeq}}%
\@y{\@s{0}%
 less-than-or-equal-to comparator for vector clocks
}%
\@xx{}%
\@x{\@s{24.53} \.{\LET} {\RECURSIVE} LTEHelper ( \_ ,\, \_ ,\, \_ )}%
\@x{\@s{49.03} LTEHelper ( vc1h ,\, vc2h ,\, index ) \.{\defeq}}%
\@x{\@s{65.43} {\IF} index \.{>} DC \.{\THEN} {\TRUE}}%
\@y{\@s{0}%
 \ensuremath{EQ
}}%
\@xx{}%
\@x{\@s{65.43} \.{\ELSE} \.{\LET} d \.{\defeq} DCIndex [ index ]}%
 \@x{\@s{96.74} \.{\IN} {\CASE} vc1h [ d ] \.{<} vc2h [ d ] \.{\rightarrow}
 {\TRUE}\@s{4.1}}%
\@y{\@s{0}%
 \ensuremath{LT
}}%
\@xx{}%
 \@x{\@s{125.34} {\Box}\@s{10.30} vc1h [ d ] \.{>} vc2h [ d ] \.{\rightarrow}
 {\FALSE}\@s{1.63}}%
\@y{\@s{0}%
 \ensuremath{GT
}}%
\@xx{}%
 \@x{\@s{125.34} {\Box}\@s{10.30} {\OTHER} \.{\rightarrow} LTEHelper ( vc1h
 ,\, vc2h ,\, index \.{+} 1 )}%
\@x{\@s{24.53} \.{\IN}\@s{4.09} LTEHelper ( vc1 ,\, vc2 ,\, 1 )}%
\@pvspace{8.0pt}%
 \@x{ KVTuple \.{\defeq} [ key \.{:} Key ,\, val \.{:} Value \.{\cup} \{
 NotVal \} ,\, vc \.{:} VC ]}%
 \@x{ OpTuple\@s{2.34} \.{\defeq} [ type \.{:} \{\@w{R} ,\,\@w{W} \} ,\, kv
 \.{:} KVTuple ,\, c \.{:} Client ,\, cnt \.{:} Nat ]}%
\@pvspace{8.0pt}%
\@x{ Message\@s{3.08} \.{\defeq}}%
 \@x{\@s{43.99} [ type \.{:} \{\@w{ReadRequest} \} ,\, key \.{:} Key ,\, vc
 \.{:} VC ,\, c \.{:} Client ,\, p \.{:} Partition ,\, d \.{:} Datacenter ]}%
 \@x{\@s{16.4} \.{\cup}\@s{16.48} [ type \.{:} \{\@w{ReadReply} \} ,\, val
 \.{:} Value \.{\cup} \{ NotVal \} ,\, vc \.{:} VC ,\, c \.{:} Client ]}%
 \@x{\@s{16.4} \.{\cup}\@s{16.48} [ type \.{:} \{\@w{UpdateRequest} \} ,\, key
 \.{:} Key ,\, val \.{:} Value ,\, vc \.{:} VC ,\, c \.{:} Client ,\, p \.{:}
 Partition ,\, d \.{:} Datacenter ]}%
 \@x{\@s{16.4} \.{\cup}\@s{16.48} [ type \.{:} \{\@w{UpdateReply} \} ,\, ts
 \.{:} Nat ,\, c \.{:} Client ,\, d \.{:} Datacenter ]}%
 \@x{\@s{16.4} \.{\cup}\@s{16.48} [ type \.{:} \{\@w{Replicate} \} ,\,
 d\@s{3.23} \.{:} Datacenter ,\, kv \.{:} KVTuple ]}%
 \@x{\@s{16.4} \.{\cup}\@s{16.48} [ type \.{:} \{\@w{Heartbeat} \} ,\, d \.{:}
 Datacenter ,\, ts\@s{2.04} \.{:} Nat ]}%
\@pvspace{8.0pt}%
\@x{ Send ( m ) \.{\defeq} msgs \.{'} \.{=} msgs \.{\cup} \{ m \}}%
 \@x{ SendAndDelete ( sm ,\, dm ) \.{\defeq} msgs \.{'} \.{=} ( msgs \.{\cup}
 \{ sm \} ) \.{\,\backslash\,} \{ dm \}}%
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq}}%
\@x{\@s{16.4} \.{\land}\@s{9.74} cvc \.{\in} [ Client \.{\rightarrow} VC ]}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} clock \.{\in} [ Partition \.{\rightarrow} [
 Datacenter \.{\rightarrow} Nat ] ]}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} pvc \.{\in} [ Partition \.{\rightarrow} [
 Datacenter \.{\rightarrow} VC ] ]}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} css\@s{1.27} \.{\in} [ Partition
 \.{\rightarrow} [ Datacenter \.{\rightarrow} VC ] ]}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} store \.{\in} [ Partition \.{\rightarrow} [
 Datacenter \.{\rightarrow} {\SUBSET} KVTuple ] ]}%
\@x{\@s{16.4} \.{\land}\@s{9.74} msgs \.{\subseteq} Message}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} incoming \.{\in} [ Partition \.{\rightarrow}
 [ Datacenter \.{\rightarrow} Seq ( Message ) ] ]}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} L \.{\in} [ Client \.{\rightarrow} Seq (
 OpTuple ) ]}%
\@x{}\midbar\@xx{}%
\@x{ Init \.{\defeq}}%
\@x{\@s{16.4} \.{\land} cvc \.{=} [ c \.{\in} Client \.{\mapsto} VCInit ]}%
 \@x{\@s{16.4} \.{\land} clock \.{=} [ p \.{\in} Partition \.{\mapsto} [ d
 \.{\in} Datacenter \.{\mapsto} 0 ] ]}%
 \@x{\@s{16.4} \.{\land} pvc \.{=} [ p \.{\in} Partition \.{\mapsto} [ d
 \.{\in} Datacenter \.{\mapsto} VCInit ] ]}%
 \@x{\@s{16.4} \.{\land} css\@s{1.27} \.{=} [ p \.{\in} Partition \.{\mapsto}
 [ d \.{\in} Datacenter \.{\mapsto} VCInit ] ]}%
 \@x{\@s{16.4} \.{\land} store \.{=} [ p \.{\in} Partition \.{\mapsto} [ d
 \.{\in} Datacenter \.{\mapsto}}%
 \@x{\@s{79.14} [ key \.{:} \{ k \.{\in} Key \.{:} KeySharding [ k ] \.{=} p
 \} ,\, val \.{:} \{ NotVal \} ,\, vc \.{:} \{ VCInit \} ] ] ]}%
\@x{\@s{16.4} \.{\land} msgs \.{=} \{ \}}%
 \@x{\@s{16.4} \.{\land} incoming \.{=} [ p \.{\in} Partition \.{\mapsto} [ d
 \.{\in} Datacenter \.{\mapsto} {\langle} {\rangle} ] ]}%
 \@x{\@s{16.4} \.{\land} L \.{=} [ c \.{\in} Client \.{\mapsto} {\langle}
 {\rangle} ]}%
\@x{}\midbar\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@pvspace{8.0pt}%
\@x{ CanIssue ( c ) \.{\defeq} \A\, m \.{\in} msgs \.{:}}%
\@y{\@s{0}%
 to ensure well-formedness of clients
}%
\@xx{}%
 \@x{\@s{16.4} m . type \.{\in} \{\@w{ReadRequest} ,\,\@w{ReadReply}
 ,\,\@w{UpdateRequest} ,\,\@w{UpdateReply} \} \.{\implies} m . c \.{\neq} c}%
\@pvspace{8.0pt}%
\@x{ Read ( c ,\, k ) \.{\defeq}}%
\@y{\@s{0}%
...
\@xx{}%
\@x{\@s{22.12} \.{\land} CanIssue ( c )}%
 \@x{\@s{22.12} \.{\land} Send ( [ type \.{\mapsto}\@w{ReadRequest} ,\, key
 \.{\mapsto} k ,\, vc \.{\mapsto} cvc [ c ] ,\,}%
 \@x{\@s{61.89} c \.{\mapsto} c ,\, p \.{\mapsto} KeySharding [ k ] ,\, d
 \.{\mapsto} ClientAttachment [ c ] ] )}%
 \@x{\@s{22.12} \.{\land} {\UNCHANGED} {\langle} cVars ,\, sVars ,\, incoming
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ ReadReply ( c ) \.{\defeq}}%
\@y{\@s{0}%
...
\@xx{}%
\@x{\@s{16.4} \.{\land} \E\, m \.{\in} msgs \.{:}}%
 \@x{\@s{31.61} \.{\land} m . type \.{=}\@w{ReadReply} \.{\land} m . c \.{=}
 c\@s{4.1}}%
\@y{\@s{0}%
 such \ensuremath{m} is unique due to well-formedness
}%
\@xx{}%
 \@x{\@s{31.61} \.{\land} cvc \.{'} \.{=} [ cvc {\EXCEPT} {\bang} [ c ] \.{=}
 Merge ( m . vc ,\, @ ) ]}%
\@x{\@s{31.61} \.{\land} msgs \.{'} \.{=} msgs \.{\,\backslash\,} \{ m \}}%
\@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} sVars ,\, incoming {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Update ( c ,\, k ,\, v ) \.{\defeq}}%
\@y{\@s{0}%
 \ensuremath{c \.{\in} Client} updates \ensuremath{k \.{\in} Key} with
 \ensuremath{v \.{\in} Value
}}%
\@xx{}%
\@x{\@s{16.4} \.{\land} CanIssue ( c )}%
 \@x{\@s{16.4} \.{\land} Send ( [ type \.{\mapsto}\@w{UpdateRequest} ,\, key
 \.{\mapsto} k ,\, val \.{\mapsto} v ,\,}%
 \@x{\@s{56.16} vc \.{\mapsto} cvc [ c ] ,\, c \.{\mapsto} c ,\, p \.{\mapsto}
 KeySharding [ k ] ,\, d \.{\mapsto} ClientAttachment [ c ] ] )}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} cVars ,\, sVars ,\, incoming
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ UpdateReply ( c ) \.{\defeq}}%
\@y{\@s{0}%
 \ensuremath{c \.{\in} Client} handles the reply to its update request
}%
\@xx{}%
\@x{\@s{16.4} \.{\land} \E\, m \.{\in} msgs \.{:}}%
 \@x{\@s{31.61} \.{\land} m . type \.{=}\@w{UpdateReply} \.{\land} m . c \.{=}
 c}%
\@y{\@s{0}%
 such \ensuremath{m} is unique due to well-formedness
}%
\@xx{}%
 \@x{\@s{31.61} \.{\land} cvc \.{'} \.{=} [ cvc {\EXCEPT} {\bang} [ c ] [ m .
 d ] \.{=} m . ts ]}%
\@x{\@s{31.61} \.{\land} msgs \.{'} \.{=} msgs \.{\,\backslash\,} \{ m \}}%
\@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} sVars ,\, incoming {\rangle}}%
\@x{}\midbar\@xx{}%
...
\@xx{}%
\@pvspace{8.0pt}%
\@x{ ReadRequest ( p ,\, d ) \.{\defeq}}%
\@y{\@s{0}%
 handle a \ensuremath{\@w{ReadRequest}
}}%
\@xx{}%
\@x{\@s{16.4} \.{\land} \E\, m \.{\in} msgs \.{:}}%
 \@x{\@s{31.61} \.{\land} m . type \.{=}\@w{ReadRequest} \.{\land} m . p \.{=}
 p \.{\land} m . d \.{=} d}%
 \@x{\@s{31.61} \.{\land} css \.{'} \.{=} [ css {\EXCEPT} {\bang} [ p ] [ d ]
 \.{=} Merge ( m . vc ,\, @ ) ]}%
 \@x{\@s{31.61} \.{\land} \.{\LET} kvs \.{\defeq} \{ kv \.{\in} store [ p ] [
 d ] \.{:}}%
\@x{\@s{105.22} \.{\land} kv . key \.{=} m . key}%
 \@x{\@s{105.22} \.{\land} \A\, dc \.{\in} Datacenter \.{\,\backslash\,} \{ d
 \} \.{:} kv . vc [ dc ] \.{\leq} css \.{'} [ p ] [ d ] [ dc ] \}}%
 \@x{\@s{63.12} lkv\@s{1.27} \.{\defeq} {\CHOOSE} kv \.{\in} kvs \.{:} \A\,
 akv \.{\in} kvs \.{:} LTE ( akv . vc ,\, kv . vc )}%
 \@x{\@s{42.72} \.{\IN} \.{\land} SendAndDelete ( [ type
 \.{\mapsto}\@w{ReadReply} ,\, val \.{\mapsto} lkv . val ,\, vc \.{\mapsto}
 lkv . vc ,\, c \.{\mapsto} m . c ] ,\, m )}%
 \@x{\@s{63.12} \.{\land} L \.{'} \.{=} [ L {\EXCEPT} {\bang} [ m . c ] \.{=}
 Append ( @ ,\, [ type \.{\mapsto}\@w{R} ,\, kv \.{\mapsto} lkv ,\, c
 \.{\mapsto} m . c ,\, cnt \.{\mapsto} Len ( @ ) \.{+} 1 ] ) ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} cVars ,\, clock ,\, pvc ,\,
 store ,\, incoming {\rangle}}%
\@pvspace{8.0pt}%
\@x{ UpdateRequest ( p ,\, d ) \.{\defeq}}%
\@y{\@s{0}%
 handle a \ensuremath{\@w{UpdateRequest}
}}%
\@xx{}%
\@x{\@s{16.4} \.{\land} \E\, m \.{\in} msgs \.{:}}%
 \@x{\@s{31.61} \.{\land} m . type \.{=}\@w{UpdateRequest} \.{\land} m . p
 \.{=} p \.{\land} m . d \.{=} d}%
\@x{\@s{31.61} \.{\land} m . vc [ d ] \.{<} clock [ p ] [ d ]\@s{4.1}}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{\@s{31.61} \.{\land} css \.{'} \.{=} [ css {\EXCEPT} {\bang} [ p ] [ d ]
 \.{=} Merge ( m . vc ,\, @ ) ]}%
 \@x{\@s{31.61} \.{\land} \.{\LET} kv \.{\defeq} [ key \.{\mapsto} m . key ,\,
 val \.{\mapsto} m . val ,\,}%
 \@x{\@s{99.16} vc\@s{1.07} \.{\mapsto} [ m . vc {\EXCEPT} {\bang} [ d ] \.{=}
 clock [ p ] [ d ] ] ]}%
 \@x{\@s{42.72} \.{\IN} \.{\land} store \.{'} \.{=} [ store {\EXCEPT} {\bang}
 [ p ] [ d ]\@s{2.22} \.{=} @ \.{\cup} \{ kv \} ]}%
 \@x{\@s{63.12} \.{\land} SendAndDelete ( [ type \.{\mapsto}\@w{UpdateReply}
 ,\, ts \.{\mapsto} clock [ p ] [ d ] ,\, c \.{\mapsto} m . c ,\, d
 \.{\mapsto} d ] ,\, m )}%
 \@x{\@s{63.12} \.{\land} incoming \.{'} \.{=} [ incoming {\EXCEPT} {\bang} [
 p ] \.{=} [ dc \.{\in} Datacenter \.{\mapsto}}%
 \@x{\@s{82.43} {\IF} dc \.{=} d \.{\THEN} @ [ dc ] \.{\ELSE} Append ( @ [ dc
 ] ,\, [ type \.{\mapsto}\@w{Replicate} ,\, d \.{\mapsto} d ,\, kv
 \.{\mapsto} kv ] ) ] ]}%
 \@x{\@s{63.12} \.{\land} L \.{'} \.{=} [ L {\EXCEPT} {\bang} [ m . c ] \.{=}
 Append ( @ ,\, [ type \.{\mapsto}\@w{W} ,\, kv \.{\mapsto} kv ,\, c
 \.{\mapsto} m . c ,\, cnt \.{\mapsto} Len ( @ ) \.{+} 1 ] ) ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} cVars ,\, clock ,\, pvc
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Replicate ( p ,\, d ) \.{\defeq}}%
\@y{\@s{0}%
 handle a \ensuremath{\@w{Replicate}
}}%
\@xx{}%
\@x{\@s{16.4} \.{\land} incoming [ p ] [ d ] \.{\neq} {\langle} {\rangle}}%
\@x{\@s{16.4} \.{\land} \.{\LET} m \.{\defeq} Head ( incoming [ p ] [ d ] )}%
\@x{\@s{27.51} \.{\IN} \.{\land} m . type \.{=}\@w{Replicate}}%
 \@x{\@s{47.91} \.{\land} store \.{'}\@s{5.46} \.{=} [ store {\EXCEPT} {\bang}
 [ p ] [ d ] \.{=} @ \.{\cup} \{ m . kv \} ]}%
 \@x{\@s{47.91} \.{\land} pvc \.{'} \.{=} [ pvc {\EXCEPT} {\bang} [ p ] [ d ]
 [ m . d ] \.{=} m . kv . vc [ m . d ] ]}%
 \@x{\@s{47.91} \.{\land} incoming \.{'} \.{=} [ incoming {\EXCEPT} {\bang} [
 p ] [ d ] \.{=} Tail ( @ ) ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} cVars ,\, cvc ,\, clock ,\,
 css ,\, L ,\, msgs {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Heartbeat ( p ,\, d ) \.{\defeq}}%
\@y{\@s{0}%
...
\@x{\@s{16.4} \.{\land} incoming [ p ] [ d ] \.{\neq} {\langle} {\rangle}}%
\@x{\@s{16.4} \.{\land} \.{\LET} m \.{\defeq} Head ( incoming [ p ] [ d ] )}%
\@x{\@s{27.51} \.{\IN} \.{\land} m . type \.{=}\@w{Heartbeat}}%
 \@x{\@s{47.91} \.{\land} pvc \.{'} \.{=} [ pvc {\EXCEPT} {\bang} [ p ] [ d ]
 [ m . d ] \.{=} m . ts ]}%
 \@x{\@s{47.91} \.{\land} incoming \.{'} \.{=} [ incoming {\EXCEPT} {\bang} [
 p ] [ d ] \.{=} Tail ( @ ) ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} cVars ,\, cvc ,\, clock ,\,
 css ,\, store ,\, L ,\, msgs {\rangle}}%
\@x{}\midbar\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{\@s{20.49} \.{\land} clock \.{'} \.{=} [ clock {\EXCEPT} {\bang} [ p ] [
 d ] \.{=} @ \.{+} 1 ]}%
 \@x{\@s{20.49} \.{\land} pvc \.{'} \.{=} [ pvc {\EXCEPT} {\bang} [ p ] [ d ]
 [ d ] \.{=} clock \.{'} [ p ] [ d ] ]}%
 \@x{\@s{20.49} \.{\land} incoming \.{'} \.{=} [ incoming {\EXCEPT} {\bang} [
 p ] \.{=} [ dc \.{\in} Datacenter \.{\mapsto}}%
 \@x{\@s{39.80} {\IF} dc \.{=} d \.{\THEN} @ [ dc ] \.{\ELSE} Append ( @ [ dc
 ] ,\, [ type \.{\mapsto}\@w{Heartbeat} ,\, d \.{\mapsto} d ,\, ts
 \.{\mapsto} pvc \.{'} [ p ] [ d ] [ d ] ] ) ] ]}%
 \@x{\@s{20.49} \.{\land} {\UNCHANGED} {\langle} cVars ,\, cvc ,\, css ,\,
 store ,\, L ,\, msgs {\rangle}}%
\@pvspace{8.0pt}%
\@x{ UpdateCSS ( p ,\, d ) \.{\defeq}}%
\@y{\@s{0}%
 update \ensuremath{css[p][d]
}}%
\@xx{}%
 \@x{\@s{16.4} \.{\land} css \.{'} \.{=} [ css {\EXCEPT} {\bang} [ p ] [ d ]
 \.{=}}%
 \@x{\@s{64.12} [ dc \.{\in} Datacenter \.{\mapsto} SetMin ( \{ pvc [ pp ] [ d
 ] [ dc ] \.{:} pp \.{\in} Partition \} ) ] ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} cVars ,\, mVars ,\, clock ,\,
 pvc ,\, store ,\, L {\rangle}}%
\@x{}\midbar\@xx{}%
\@x{ Next \.{\defeq}}%
 \@x{\@s{16.4} \.{\lor} \E\, c\@s{0.57} \.{\in} Client ,\, k \.{\in} Key \.{:}
 Read ( c ,\, k )}%
 \@x{\@s{16.4} \.{\lor} \E\, c\@s{0.57} \.{\in} Client ,\, k \.{\in} Key ,\, v
 \.{\in} Value \.{:} Update ( c ,\, k ,\, v )}%
 \@x{\@s{16.4} \.{\lor} \E\, c\@s{0.57} \.{\in} Client \.{:} ReadReply ( c )
 \.{\lor} UpdateReply ( c )}%
 \@x{\@s{16.4} \.{\lor} \E\, p \.{\in} Partition ,\, d \.{\in} Datacenter
 \.{:}}%
\@x{\@s{31.61} \.{\lor} ReadRequest ( p ,\, d )}%
\@x{\@s{31.61} \.{\lor} UpdateRequest ( p ,\, d )}%
\@x{\@s{31.61} \.{\lor} Replicate ( p ,\, d )}%
\@x{\@s{31.61} \.{\lor} Heartbeat ( p ,\, d )}%
\@x{\@s{31.61} \.{\lor} Tick ( p ,\, d )}%
\@x{\@s{31.61} \.{\lor} UpdateCSS ( p ,\, d )}%
\@pvspace{8.0pt}%
...
\@xx{}%
\@x{\@s{16.4} \.{\LET} {\RECURSIVE} ValidHelper ( \_ ,\, \_ )}%
\@x{\@s{40.89} ValidHelper ( seq ,\, kvs ) \.{\defeq}}%
\@x{\@s{57.29} {\IF} seq \.{=} {\langle} {\rangle} \.{\THEN} {\TRUE}}%
\@x{\@s{57.29} \.{\ELSE} \.{\LET} op \.{\defeq} Head ( seq )}%
\@x{\@s{88.61} \.{\IN} {\IF} op . type \.{=}\@w{W}\@s{131.2}}%
\@y{\@s{0}%
 overwritten
}%
\@xx{}%
 \@x{\@s{109.01} \.{\THEN} ValidHelper ( Tail ( seq ) ,\, op . kv . key
 \.{\colongt} op . kv . vc \.{\,@@\,} kvs )}%
\@x{\@s{109.01} \.{\ELSE} \.{\land} op . kv . vc \.{=} kvs [ op . kv . key ]}%
\@x{\@s{140.32} \.{\land} ValidHelper ( Tail ( seq ) ,\, kvs )}%
 \@x{\@s{16.4} \.{\IN}\@s{4.09} ValidHelper ( s ,\, [ k \.{\in} Key
 \.{\mapsto} VCInit ] )\@s{4.1}}%
\@y{\@s{0}%
 with initial values
}%
...
\@xx{}%
 \@x{\@s{21.86} \.{\LET} ops \.{\defeq} {\UNION} \{ Range ( L [ c ] ) \.{:} c
 \.{\in} Client \}}%
 \@x{\@s{42.26} rops\@s{2.93} \.{\defeq} \{ op \.{\in} ops \.{:} op .
 type\@s{11.23} \.{=}\@w{R} \}}%
 \@x{\@s{42.26} wops \.{\defeq} \{ op \.{\in} ops \.{:} op . type\@s{11.23}
 \.{=}\@w{W} \}}%
 \@x{\@s{42.26} so\@s{4.09} \.{\defeq} {\UNION} \{ SeqToRel ( L [ c ] ) \.{:}
 c \.{\in} Client \}}%
\@y{\@s{0}%
 session order
}%
\@xx{}%
 \@x{\@s{42.26} rf\@s{4.52} \.{\defeq} \{ {\langle} w ,\, r {\rangle} \.{\in}
 wops \.{\times} rops \.{:} w . kv . key \.{=} r . kv . key \.{\land} w . kv
 . vc \.{=} r . kv . vc \}}%
\@x{\@s{46.36} co \.{\defeq} TC ( so \.{\cup} rf )}%
\@y{\@s{0}%
 causality order
}%
\@xx{}%
\@x{\@s{21.86} \.{\IN}\@s{4.09} \A\, c \.{\in} Client \.{:}}%
 \@x{\@s{57.68} \E\, sc \.{\in} PermutationsOf ( L [ c ] \.{\circ} SetToSeq (
 wops ) ) \.{:}}%
\@x{\@s{69.00} \.{\land} Valid ( sc )}%
\@x{\@s{69.00} \.{\land} Respect ( sc ,\, co )}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} Spec \.{\implies} {\Box} CM}%
\@x{}\bottombar\@xx{}%
\end{document}
